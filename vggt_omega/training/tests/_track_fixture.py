"""Synthetic track supervision for trainer tests.

Verbatim restore of vggt_omega/datasets/track_util.py, which PR #12 removed
from the production tree (vendors supply real tracks now; composed_dataset
guards the load_track=True path with a runtime error). test_trainer still
needs to SYNTHESIZE track supervision for the matching-loss path, so the
builder lives on here as a test fixture rather than a public dataset API.
"""
import os
