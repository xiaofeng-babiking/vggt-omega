...
class SamplerContract:
    ...
    def test_return_contract(self, seed):
        ...
        idx, dist = sample_se3_trajectory(poses, num=5, start=0, end=11)
    ...
    def test_respects_subrange(self, seed):
        ...
        idx, _ = sample_se3_trajectory(poses, num=4, start=3, end=10)
    ...
    def test_default_window_is_full(self):
        ...
        idx, _ = sample_se3_trajectory(poses, num=4)  # start=0, end=-1
        assert idx[0] == 0 and idx[-1] == 7

    def test_negative_end_index(self):
        rng = _rng(0)
        poses = [self.random_pose(rng) for _ in range(10)]
        assert sample_se3_trajectory(poses, num=3, start=0, end=-1)[0][-1] == 9
        assert sample_se3_trajectory(poses, num=3, start=0, end=-3)[0][-1] == 7

    def test_num_one_returns_start(self):
        rng = _rng(0)
        poses = [self.random_pose(rng) for _ in range(6)]
        idx, dist = sample_se3_trajectory(poses, num=1, start=2, end=5)
        assert idx == [2] and dist == [0.0]

    def test_num_equals_window_returns_every_frame(self):
        rng = _rng(0)
        poses = [self.random_pose(rng) for _ in range(6)]
        idx, dist = sample_se3_trajectory(poses, num=4, start=1, end=4)
        assert idx == [1, 2, 3, 4]
    ...
    def test_asserts_window_at_least_num(self):
        ...
        with pytest.raises(AssertionError):
            sample_se3_trajectory(poses, num=5, start=0, end=3)  # window 4 < 5

    def test_rejects_nonpositive_num(self):
        rng = _rng(0)
        poses = [self.random_pose(rng) for _ in range(5)]
        with pytest.raises(ValueError):
            sample_se3_trajectory(poses, num=0)

    def test_rejects_end_before_start(self):
        rng = _rng(0)
        poses = [self.random_pose(rng) for _ in range(6)]
        with pytest.raises(ValueError):
            sample_se3_trajectory(poses, num=2, start=4, end=1)

    def test_rejects_out_of_range_window(self):
        rng = _rng(0)
        poses = [self.random_pose(rng) for _ in range(5)]
        with pytest.raises(ValueError):
            sample_se3_trajectory(poses, num=2, start=0, end=99)

    def test_rejects_empty(self):
        with pytest.raises(ValueError):
            sample_se3_trajectory([], num=1)
    ...
    def test_skips_small_motion_frames(self, seed):
        ...
        idx, _ = sample_se3_trajectory(poses, num=5, start=0, end=len(poses) - 1)
    ...
    def test_equal_arclength_spacing(self, seed):
        ...
        num = 5
        idx, dist = sample_se3_trajectory(poses, num=num, start=0, end=len(poses) - 1)
    ...
    def test_dedup_removes_duplicate_snaps(self, seed):
        ...
        idx, dist = sample_se3_trajectory(poses, num=8, start=0, end=len(poses) - 1)
    ...
    def test_no_motion_spreads_by_index(self):
        ...
        idx, dist = sample_se3_trajectory([p] * 9, num=5, start=0, end=8)
    ...
    def test_deterministic_for_fixed_window(self, seed):
        rng = _rng(seed)
        poses = [self.random_pose(rng) for _ in range(12)]
        a = sample_se3_trajectory(poses, num=5, start=1, end=10)
        b = sample_se3_trajectory(poses, num=5, start=1, end=10)
        assert a == b
